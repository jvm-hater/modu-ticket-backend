    ...
    init {
        describe("#create"){
            context("새로운 id의 유저라면"){
                ...
                it("테이블에 값을 저장한다"){
                    userRepository.find("test1").id shouldBe "test1"
                }
            }
        ...
        describe("#find"){
            context("테이블에 있는 유저라면"){
                userRepository.create(id = "test1", password = "testPassword")
                it("해당 유저를 리턴한다."){
                    userRepository.find("test1") shouldBe User(id = "test1", point = 0, rank = Rank.BRONZE)
                }
            }
        ...
        describe("#findAll"){
            context("테이블에 있는 유저라면"){
                userRepository.create(id = "test1", password = "testPassword")
                it("해당 유저를 삭제한다."){
                    userRepository.delete("test1") shouldBe Unit
                }
            }
            context("해당 유저가 존재하지 않는다면"){
                it("삭제를 실패한다."){
