...
import com.jvmhater.moduticket.dto.response.TokenResponse
...
class AuthController(val authService: AuthService) {
    ...
    suspend fun login(@RequestBody request: LoginRequest): TokenResponse {
        return TokenResponse(authService.login(userId = request.id, password = request.password))
    }

    @Operation(description = "회원가입 한다.")
    @PostMapping("/signup")
    suspend fun signUp(@RequestBody request: SignUpRequest) {
        authService.signUp(userId = request.id, password = request.password)
    }
}
